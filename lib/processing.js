...
module.exports = {
	toTokenTable: data => {
		var table = new Table({
			head: [
				chalk.blueBright.bold("Address"),
				chalk.blueBright.bold("Ammount"),
			],
			colWidths: [60, 30],
		});
	...
	saveTxConfig: async data => {
		...
		const file =
			...
			"/aragon-toolkit-examples/examples/mintAndBurn/assignations.json";
		const obj = {
	...
	submitTx: async () => {
		...
		try {
			...
			const com = await execa.command(
				"node /home/m-root/projects/powertools/aragon-toolkit-examples/examples/mintAndBurn/index.js | sh",
				{
					shell: true,
				},
			);
