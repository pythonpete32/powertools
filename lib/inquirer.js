...
const refresh = async () => {
	...
	console.log(chalk.blue.bold("For Aragon Power Users"));
};
...
module.exports = {
	askDaoAddresses: async () => {
		...
		const questions = [{
			...
			},
			{
				...
				type: "input",
				message: "Enter your " +
					chalk.white.bold("Token Manager") +
					" address:",
				validate: function (value) {
					if (value.length == 42) {
						return true;
	...
	getMints: async () => {
		...
		while (more) {
			...
			const questions = [{
					name: "reciever",
					type: "input",
					message: chalk.yellow(count) +
						chalk.redBright.bold(" Recipient") +
						" address:",
					validate: function (value) {
						if (value.length == 42) {
							return true;
	...
	login: async () => {
		...
		const questions = [{
			...
			choices: [
				chalk.yellowBright("Ocean Community DAO"),
				new inquirer.Separator(),
				"new",
			],
		}, ];
	...
	menu: async () => {
		...
		const questions = [{
			...
			choices: [{
					name: chalk.green("Super Mint:      ") +
						"   mints tokens to multiple addresses",
					value: "mint",
				},
				{
					name: chalk.green("SourceCred:      ") +
						"   Calculate cred and mint grain in your DAO.",
					value: "sourcecred",
				},
				{
					name: chalk.green("Atomic payments: ") +
						"   Pay multiple people with just one vote.",
					value: "payments",
				},
				{
					name: chalk.green("Swap SAI to DAI: ") +
						"   Swap SAI in a Vault/Agent into DAI with just one vote.",
					value: "swap",
				},
				{
					name: chalk.green("Permissions:     ") +
						"   Create, grant, and revoke many permissions with just one vote.",
					value: "permissions",
				},
				new inquirer.Separator(),
