...
module.exports = {
	mode: async () => {
		...
		const questions = [{
				...
				type: "input",
				message: "Enter your " +
					chalk.white.bold("Discourse Forum") +
					" address:",
			...
			},
			{
				...
				choices: [{
						name: chalk.greenBright("Mint Grain in DAO"),
						value: "mint",
					},
					new inquirer.Separator(),
					{
						name: chalk.greenBright("Save Cred to .CSV"),
						value: "save",
					},
				],
			},
	...
	startBackend: async () => {
		...
		spinner.succeed("SourceCred Backend " + chalk.greenBright("UP"));
	},
	runSC: async url => {
		const spinner = ora();
		try {
			...
			await execa.command(
				`node bin/sourcecred.js discourse https://${url}`,
			);
		} catch (error) {
			spinner.fail();
			console.error(error);
	...
	calcCred: async () => {
		...
		try {
			...
			await execa.command(
				`node bin/sourcecred.js scores port.oceanprotocol.com > CRED.json`, {
					shell: true,
				},
			);
	...
	processCSV: async () => {
		try {
			...
			actual = [];
			data[1].users.map(element => {
				actual.push([addresses.pop(), element.totalCred]);
			});
			*/
			...
			//const csvContent = actual.map(e => e.join(",")).join("\n");
			/*
